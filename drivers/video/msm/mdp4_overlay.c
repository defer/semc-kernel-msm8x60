	...
	for (i = 0; i < MDP4_MAX_PIPE; i++) {
		if (pipe->pipe_type == ptype && pipe->pipe_used == 0) {
			...
			if (pd->share) { /* pipe can be shared */
				if ((pipe->pipe_num % MDP4_MIXER_MAX) !=
						mixer) {
					pipe++;
					continue;
				}
...
int mdp4_overlay_get(struct fb_info *info, struct mdp_overlay *req)
{
	...
	pipe->req_data.user_data[7] = (mfd) ? mfd->panel_power_on : TRUE;
...
#ifdef CONFIG_MSM_BUS_SCALING
	if ((pipe->mixer_num == MDP4_MIXER0) &&
		(!(ctrl->ov_pipe[OVERLAY_PIPE_VG1].ref_cnt +
			ctrl->ov_pipe[OVERLAY_PIPE_VG2].ref_cnt)))
		if (mfd->panel_power_on)
...
int mdp4_overlay_refresh(struct fb_info *info, int ndx)
{
	...
	if (mdp4_overlay_stage_pipe(pipe->mixer_num, pipe->mixer_stage)
			!= pipe) {
		mutex_unlock(&mfd->dma->ov_mutex);
		return -ENODEV;
	}
