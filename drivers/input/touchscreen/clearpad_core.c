...
static int synaptics_clearpad_prepare_f11_2d(struct synaptics_clearpad *this)
{
	...
	if (rc)
		return rc;
	buf[0] |= XY_REPORTING_MODE_REDUCED_REPORTING_MODE;
	rc = synaptics_put(this, SYNF(F11_2D, CTRL, 0x00), buf[0]);
	if (rc)
		return rc;
		...
		*this)
{
	...
	this->charger_mode_status = this->wall_charger_status;
	dev_info(&this->pdev->dev, "%s: Set charger mode: %d\n",
		__func__, this->charger_mode_status);

	return rc;
}

static void synaptics_clearpad_charger_status_update(struct synaptics_clearpad
		*this, bool new_status)
{
	dev_info(&this->pdev->dev, "%s: Wall charger status: %d => %d\n",
		__func__, this->wall_charger_status, new_status);
	this->wall_charger_status = new_status;

	/* set charger mode if necessary */
	if (this->charger_mode_status != this->wall_charger_status) {
		synaptics_clearpad_charger_mode_update(this);
	}
}
	...
	for (; funcarea->func != SYN_FUNCAREA_END; funcarea++) {
		...
		case SYN_FUNCAREA_POINTER:
			input_set_abs_params(this->input, ABS_MT_POSITION_X,
					     funcarea->original.x1,
					     funcarea->original.x2, 0, 0);
			input_set_abs_params(this->input, ABS_MT_POSITION_Y,
					     funcarea->original.y1,
					     funcarea->original.y2, 0, 0);
				    ...
				    struct synaptics_pointer *pointer)
{
	...
	case SYN_FUNCAREA_POINTER:
		...
		width_major = max(cur->wx, cur->wy) + 1;
		width_minor = min(cur->wx, cur->wy) + 1;
		touch_major = cur->z;
		input_report_abs(this->input, ABS_MT_TRACKING_ID, cur->id);
...
static void synaptics_clearpad_worker(struct work_struct *work)
{
	...
	if (interrupt & this->pdt[SYN_F11_2D].irq_mask) {
		...
		(DEVICE_STATUS_UNCONFIGURED_DEVICE_FAILURE == status)) {
			synaptics_put(this, SYNF(F01_RMI, COMMAND, 0x00),
					DEVICE_COMMAND_RESET);
			goto unlock;
		}
		...
		for (i = 0; i < this->extents.n_fingers; ++i)
			synaptics_report_finger_n(this, i, buf);
		...
		const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, TOUCHCMD_CHARGER_MODE_START)) {
		synaptics_clearpad_charger_status_update(this, true);
	} else if (sysfs_streq(buf, TOUCHCMD_CHARGER_MODE_END)) {
		synaptics_clearpad_charger_status_update(this, false);
	} else {
		...
		u8 mode, u8 count)
{
	...
	if (this->pdt[SYN_F05_ANALOG].number !=
		...
		dev_err(&this->pdev->dev, "F05 is not initilised\n");
		goto err_retrun;
	}
	...
	rc = synaptics_put(this, SYNF(F01_RMI, CTRL, 0x01),
			this->pdt[SYN_F05_ANALOG].irq_mask);
	if (rc)
		goto set_power;
	...
	rc = synaptics_read(this, SYNF(F05_ANALOG, QUERY, 0x01), &buf, 1);
	if (rc)
		goto err_set_page_sel_0;
	num_tx = buf & ANALOG_QUERY_NUM_OF_ELECTRODES;

	rc = synaptics_read(this, SYNF(F05_ANALOG, QUERY, 0x00), &buf, 1);
	if (rc)
		goto err_set_page_sel_0;
	num_rx = buf & ANALOG_QUERY_NUM_OF_ELECTRODES;
	...
	for (k = 0; k < count; k++) {
		...
		dev_info(&this->pdev->dev,
				"ANALOG: mode = [0x%02x], num = [%d]", mode, k);
		for (i = 0; i < num_tx; i++) {
			rc = synaptics_put(this,
					SYNF(F05_ANALOG, DATA, 0x01), mode + i);
			if (rc)
				goto err_reset;
			...
			rc = synaptics_read(this, SYNF(F05_ANALOG, DATA, 0x02),
					data, num_rx * 2);
			if (rc)
				goto err_reset;
		...
		const char __user *buf, size_t count, loff_t *pos)
{
	...
	case 'W':
		...
		break;
	case 'S':
		synaptics_clearpad_analog_test(this, HWTEST_SIGNAL_DIFF, arg);
		break;
	case 'B':
		synaptics_clearpad_analog_test(this, HWTEST_BASELINE, arg);
		break;
	default:
		break;
	}
